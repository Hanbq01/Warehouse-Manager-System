...
public class Login extends JFrame {
	...
	private void init() {
				...
				if (JB2.isSelected()) {
					...
					try {
						...
						if (rs.next()) {
							...
							SystemUserFrame.USER_NAME = name;
						} else {
					...
					try {
						...
						if (rs.next()) {
							JOptionPane.showConfirmDialog(Login.this,
									...
									JOptionPane.CLOSED_OPTION);
							dispose();
							SystemRootFrame LR = new SystemRootFrame();
							LR.setVisible(true);
							SystemRootFrame.USER_NAME = name;
						} else {
							JOptionPane.showMessageDialog(null, "账号或密码错误！");
						}
					} catch (SQLException e1) {
		...
		panMain.add(JB1);
		panMain.add(JB2);
		ImageIcon img = new ImageIcon("image/666.jpg");
		// 要设置的背景图片
		JLabel imgLabel = new JLabel(img);
		// 将背景图放在标签里。
		panMain.add(imgLabel, new Integer(Integer.MIN_VALUE));
		// 将背景标签添加到jfram的LayeredPane面板里。
		imgLabel.setBounds(-65, -55, img.getIconWidth(), img.getIconHeight());
		// 设置背景标签的位置
